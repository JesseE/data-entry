	...
	click:function(ev,rightClick){
		if(!game.paused){
			...
			if (!rightClick){ // Player left clicked
				...
				var uids = [];			
				if (clickedItem){ 
				// Player right clicked on something... Specific action
					if (clickedItem.type != "terrain"){	
						if (clickedItem.team != game.team){ // Player right clicked on an enemy item
							...
							// Laat de harvester resources droppen als de depot geklikt wordt
							if(clickedItem.name == "depot" && item.canHarvest && item.resources > 0){
								game.sendCommand(uids, {type:"returnResources", toUid:clickedItem.uid, returnManual:true});
					...
					} else if (clickedItem.group == "resource"){
						for (var i = game.selectedItems.length - 1; i >= 0; i--){
							var item = game.selectedItems[i];
							// pick the first selected harvester since only one can deploy at a time
							if(item.team == game.team && (item.type == "vehicles" && item.name == "nanobot")){
								uids.push(item.uid);
								break;
							}
						};
						if (uids.length>0){
							game.sendCommand(uids,{type:"harvest",toUid:clickedItem.uid});
							sounds.play("acknowledge-moving");
						}
					} else if (clickedItem.group == "teleporter"){
						moveUnits();
					}
				...
				function moveUnits(){
					...
					for (var i = game.selectedItems.length - 1; i >= 0; i--){
						...
						if(item.team == game.team && (item.type == "vehicles" || item.type == "aircraft")){
							uids.push(item.uid);
						}else if(item.team == game.team && (item.name == "barrack-light" || item.name == "barrack-heavy")){
							barracks.push(item.uid);
						}
					};
					if (uids.length>0){
					...
					if (barracks.length > 0){
						game.sendCommand(barracks,{type:'setWaypoint',to:{x:mouse.gameX/game.gridSize,y:mouse.gameY/game.gridSize}});
						sounds.play("acknowledge-moving");
					}
				}
			}
    ...
    init:function(){
		...
		$mouseCanvas.mouseup(function(ev) {
			...
			//Left key was released
				if(!shiftPressed && mouse.buttonPressed && game.selectedItems.length > 0){
					if  ((Math.abs(mouse.dragX - mouse.gameX) < 4 ||
				          Math.abs(mouse.dragY - mouse.gameY) < 4)){
				            game.clearSelection();
					}
